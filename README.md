...
## 与 Loop Engineering / Boris Loop 的关系
...
Loop Engine 不直接对应 Boris 三层架构中的任何一层——它是在更早的**设计阶段**介入的：
...
加入 Loop Engine 后的流程：
  ┌────────────────┐     ┌──────────────────┐     ┌──────────────────────┐
  │ 模糊需求         │ →   │ Loop Engine       │ →   │ 动态 Workflow /     │
  │                 │     │ 设计 + 门禁 + 循环  │     │ /loop / Routines    │
  └────────────────┘     └──────────────────┘     └──────────────────────┘
                               │
                               ├── Gate → 对应 Boris 的**写查分离**（独立 subagent 评估，不自我打分）
                               ├── Loop → 对应 Boris 的**递归式终止**思想（带反馈的迭代，不是空转）
                               └── best_of → 对应 Boris 的**记忆进化**理念（不假设最后一轮最好）
```
...
| **best_of 收敛** | —（Boris 未显式提出，但其系统通过 CLAUDE.md 蒸馏实现长效记忆） | Loop Engine 特有：不默认"最后一轮最好"，维护 best-so-far 指针 |
| **数据模式（overwrite/accumulate/best_of）** | 文件系统层间通信 | Boris 的层间通过写文件通信；Loop Engine 的 data_mode 定义了同一层内的迭代数据管理 |
| **double_check** | Evaluator 方差意识 | Boris 的系统用独立小模型做 evaluación；Loop Engine 明确指出"同一个模型跑两次只防噪声不防偏差" |
| **防执行幻觉纪律** | —（Boris 的工作基于 Claude Code 产品内置的 auto-mode，不依赖模型自觉） | Loop Engine 因是 prompt 模板而非产品，必须靠显式纪律补偿 |

### 本质关系

Loop Engine 和 Boris Loop 的关系可以用一句话概括：

> **Boris Loop 是产品层面的实现（三层架构 + 原生命令），Loop Engine 是设计层面的方法论（先思考再执行的纪律 + 门禁驱动的质量闭环）。** 两者面向同一问题——让 AI 在循环中自主工作——但 Boris 从运行时向下设计（有了基础设施，再定义使用方式），Loop Engine 从流程向上设计（先定义纪律，再约束执行）。当 Claude Code 的原生命令（/loop、/goal、Routines）作为基础设施就位时，Loop Engine 的 YAML 配置可以通过动态 Workflow 直接调用它们——设计层和执行层就打通了。
